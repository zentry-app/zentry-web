...
// Dynamic Components
const DetallesIngresoDialogContent = dynamic(
  () => import("@/components/dashboard/ingresos/DetallesIngresoDialogContent"),
  {
    loading: () => (
      <div className="p-10 text-center">
        <RefreshCw className="h-8 w-8 animate-spin mx-auto mb-4" /> Cargando
        auditoría...
      </div>
    ),
    ssr: false,
...
const VistaPorCasa = dynamic(
  () => import("@/components/dashboard/ingresos/VistaPorCasa"),
  {
    loading: () => (
      <div className="p-6">
...
const IngresosPorCasaDialog = dynamic(
  () => import("@/components/dashboard/ingresos/IngresosPorCasaDialog"),
  {
    ssr: false,
  },
);
...
const PaginationControls = dynamic(
  () => import("@/components/dashboard/ingresos/PaginationControls"),
  {
    ssr: false,
  },
);

const EstadisticasIngresos = dynamic(
  () => import("@/components/dashboard/ingresos/EstadisticasIngresos"),
  {
    loading: () => (
      <div className="p-20 text-center">
        <RefreshCw className="h-10 w-10 animate-spin mx-auto mb-4 text-primary" />
        <p className="font-black text-slate-400">
          Generando reporte de analítica...
        </p>
      </div>
    ),
    ssr: false,
...
export default function IngresosMasterPage() {
  ...
  const adminResidencialId =
    userClaims?.managedResidencialId || userClaims?.residencialId;
  ...
  const formatDateToRelative = useCallback((ts: any) => {
    try {
      return formatDistanceToNow(convertToDate(ts), {
        addSuffix: true,
        locale: es,
      });
    } catch (e) {
  ...
  useEffect(() => {
    const fetchResidenciales = async () => {
      try {
        const docs = await getCachedResidenciales();
        const list = docs.map(
          (doc) => ({ id: doc.id, ...doc.data() }) as Residencial,
        );
        ...
        if (!isGlobalAdmin && adminResidencialId) {
          const found = list.find(
            (r) =>
              r.residencialID === adminResidencialId ||
              r.id === adminResidencialId,
          );
          if (found) {
      ...
      if (isGlobalAdmin) {
        targets =
          residencialFilter === "todos"
            ? residenciales.map((r) => r.id)
            : [residencialFilter];
      } else if (adminResidencialId) {
        const myRes = residenciales.find(
          (r) =>
            r.residencialID === adminResidencialId ||
            r.id === adminResidencialId,
        );
        targets = myRes ? [myRes.id] : [];
        if (myRes && residencialFilter !== myRes.id)
          setResidencialFilter(myRes.id);
      }
      ...
      targets.forEach((resId) => {
        ...
        if (monthFilter !== "todos") {
          ...
          queryConstraints.push(
            where("timestamp", ">=", FirestoreTimestamp.fromDate(startDate)),
          );
          queryConstraints.push(
            where("timestamp", "<=", FirestoreTimestamp.fromDate(endDate)),
          );
        } else {
        ...
        const unsub = onSnapshot(
          ...
          (snap) => {
            const resNombre =
              residenciales.find((r) => r.id === resId)?.nombre ||
              "Residencial";
            const nuevos = snap.docs.map(
              (doc) =>
                ({
                  id: doc.id,
                  ...doc.data(),
          ...
          (err) => {
            ...
            if (err.message?.includes("index")) {
              toast.error(
                "Error de base de datos. Se requiere crear un índice.",
              );
            }
            setLoading(false);
          },
  ...
  const filteredIngresos = useMemo(() => {
    return ingresos.filter((ing) => {
      ...
      const domicilio =
        `${ing.domicilio?.calle || ""} ${ing.domicilio?.houseNumber || ""}`.toLowerCase();
      ...
      const matchesTipo =
        ...
        (tipoIngresoFilter === "autorizada"
          ? ing.category !== "temporal" &&
            ing.category !== "evento" &&
            ing.category !== "moroso_entry"
          : ing.category === tipoIngresoFilter);

      const matchesStatus =
        statusFilter === "todos" || ing.status === statusFilter;
      ...
      if (monthFilter && monthFilter !== "todos") {
        ...
        matchesMonth =
          ingDate.getFullYear() === year && ingDate.getMonth() === targetMonth;
      }

      let matchesSpecificDate = true;
      if (dateFilter) {
        const target = parseISO(dateFilter);
        matchesSpecificDate = isWithinInterval(ingDate, {
  ...
  const stats = useMemo(
    () => ({
      ...
      casasAnalizadas: new Set(
        filteredIngresos.map(
          (i) => `${i.domicilio?.calle}#${i.domicilio?.houseNumber}`,
        ),
      ).size,
    }),
    [filteredIngresos],
    ...
    <div className="min-h-screen bg-premium p-4 lg:p-10 space-y-8 pb-20">
      ...
      >
        <div className="space-y-4 max-w-2xl">
          ...
          <p className="text-slate-600 font-bold text-base sm:text-lg max-w-lg">
            Control total de ingresos y auditoría mensual avanzada del recinto.
          </p>
        </div>
      ...
      <div className="flex flex-col gap-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
          <StatTile
            icon={<User />}
            label={`Ingresos ${monthFilter === "todos" ? "Totales" : "Mes"}`}
            value={stats.total}
            color="blue"
          ...
          <StatTile
            icon={<Car />}
            label="Vehiculares"
            value={stats.vehiculares}
            color="purple"
            description="Con placa auditada"
          />
          <StatTile
            icon={<Home />}
            label="Cobertura"
            value={stats.casasAnalizadas}
            color="orange"
            description="Casas visitadas"
          />
        </div>
      </div>
          ...
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
            ...
            <div className="flex flex-col sm:flex-row items-center gap-4">
              <Tabs
                ...
                className="bg-slate-200/50 p-1 rounded-xl sm:rounded-2xl shadow-inner w-full sm:w-auto"
              >
                <TabsList className="bg-transparent border-none gap-1 flex justify-center">
                  <TabsTrigger
                    value="general"
                    className="flex-1 sm:flex-none h-9 px-4 sm:px-6 rounded-lg sm:rounded-xl font-black text-[9px] sm:text-[10px] uppercase tracking-widest data-[state=active]:bg-slate-900 data-[state=active]:text-white data-[state=active]:shadow-lg text-slate-700"
                  >
                    <ArrowRightLeft className="w-3.5 h-3.5 mr-2 hidden sm:inline" />{" "}
                    Historial
                  </TabsTrigger>
                  <TabsTrigger
                    value="casas"
                    className="flex-1 sm:flex-none h-9 px-4 sm:px-6 rounded-lg sm:rounded-xl font-black text-[9px] sm:text-[10px] uppercase tracking-widest data-[state=active]:bg-slate-900 data-[state=active]:text-white data-[state=active]:shadow-lg text-slate-700"
                  >
                    <Home className="w-3.5 h-3.5 mr-2 hidden sm:inline" />{" "}
                    Agrupado
                  </TabsTrigger>
                  <TabsTrigger
                    value="analitica"
                    className="flex-1 sm:flex-none h-9 px-4 sm:px-6 rounded-lg sm:rounded-xl font-black text-[9px] sm:text-[10px] uppercase tracking-widest data-[state=active]:bg-slate-900 data-[state=active]:text-white data-[state=active]:shadow-lg text-slate-700"
                  >
                    <TrendingUp className="w-3.5 h-3.5 mr-2 hidden sm:inline" />{" "}
                    Analítica
                  </TabsTrigger>
                </TabsList>
              </Tabs>
              {(searchTerm ||
                dateFilter ||
                (isGlobalAdmin && residencialFilter !== "todos") ||
                statusFilter !== "todos") && (
                <Button
                  variant="secondary"
                  onClick={clearFilters}
                  className="rounded-xl sm:rounded-2xl font-black text-[10px] uppercase tracking-widest h-11 px-6 shadow-sm bg-white border border-slate-200 hover:bg-slate-50 text-slate-700 w-full sm:w-auto"
                >
                  ...
                  <Table>
                    <TableHeader className="bg-slate-100/50">
                      <TableRow className="border-none">
                        <TableHead className="py-6 px-10 text-[10px] font-black uppercase tracking-[0.2em] text-slate-700">
                          ID Pase
                        </TableHead>
                        <TableHead className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-700">
                          Identificación / Tipo
                        </TableHead>
                        <TableHead className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-700">
                          Vehículo / Placa
                        ...
                        <TableHead className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-700 text-right px-10">
                          Estado
                        </TableHead>
                      </TableRow>
                    </TableHeader>
                            ...
                            <TableCell>
                              <div className="flex items-center gap-4">
                                ...
                                <div className="min-w-0">
                                  <p className="font-black text-slate-900 truncate max-w-[180px]">
                                    {ing.visitData?.name || "---"}
                                  </p>
                                  <Badge
                                    className={`text-[9px] h-5 rounded-md px-2 font-black uppercase tracking-widest border-none shadow-sm ${
                                      ing.category === "temporal"
                              ...
                              {ing.vehicleInfo?.placa ? (
                                <div className="space-y-1">
                                  ...
                                  <p className="text-[10px] font-black uppercase text-slate-500">
                                    {ing.vehicleInfo.marca}{" "}
                                    {ing.vehicleInfo.modelo}
                                  </p>
                                </div>
                              ) : (
                                <Badge
                                  variant="outline"
                                  className="border-slate-300 text-slate-600 font-black text-[9px] uppercase tracking-widest bg-slate-50 px-2 py-1"
                                >
                                  Peatonal
                                </Badge>
                              )}
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                <div>
                                  <p className="font-black text-sm text-slate-900">
                                    {ing.domicilio?.calle} #
                                    {ing.domicilio?.houseNumber}
                                  </p>
                                  <p className="text-[10px] font-bold text-slate-500 uppercase tracking-tighter">
                                    {ing._residencialNombre}
                                  </p>
                                </div>
                              </div>
                            </TableCell>
                            ...
                            <TableCell className="text-right px-10">
                              <div className="flex items-center justify-end gap-3">
                                <Badge
                                  className={
                                    ing.status === "active"
                                      ? "bg-emerald-600 text-white border-none px-4 py-1.5 rounded-full font-black text-[9px] tracking-widest shadow-md"
                                      : ing.status === "completed"
                                        ? "bg-blue-600 text-white border-none px-4 py-1.5 rounded-full font-black text-[9px] tracking-widest shadow-md"
                              ...
                              <div className="space-y-1">
                                <p className="text-xl font-black text-slate-600">
                                  ...
                                  {monthFilter === "todos"
                                    ? "el Historial"
                                    : format(
                                        parseISO(monthFilter + "-01"),
                                        "MMMM",
                                        { locale: es },
                                      )}
                                </p>
                                <p className="text-sm text-slate-500 font-bold">
                                  Intenta ajustar los filtros de periodo o
                                  categoría.
                                </p>
                              </div>
                              <Button
                                onClick={clearFilters}
                                variant="outline"
                                className="rounded-2xl font-black mt-2 text-slate-700 border-slate-300"
                              >
                                Limpiar Todo
                              </Button>
                            </div>
              ...
              >
                <VistaPorCasa
                  ...
                  onVerDetalles={(casa) => {
                    ...
                    setCasaDialogOpen(true);
                  }}
                  getResidencialNombre={(id) =>
                    residenciales.find((r) => r.id === id)?.nombre || "---"
                  }
                />
              </motion.div>
            ) : (
  ...
  return (
    <Card className="border-none shadow-zentry-lg bg-white/70 backdrop-blur-2xl rounded-2xl sm:rounded-[2.2rem] p-4 sm:p-6 group hover:translate-y-[-4px] hover:shadow-2xl transition-all duration-300 ring-1 ring-slate-100 min-w-0">
      <div className="flex items-start gap-3 sm:gap-5">
        <div
          className={`h-10 w-10 sm:h-14 sm:w-14 rounded-xl sm:rounded-2xl ${colors[color]} border flex items-center justify-center shrink-0 group-hover:scale-110 group-hover:rotate-3 transition-all shadow-sm`}
        >
          {React.cloneElement(icon, { className: "h-5 w-5 sm:h-7 sm:w-7" })}
        </div>
        <div className="space-y-0.5 min-w-0">
          <p className="text-[9px] sm:text-[10px] font-black text-slate-600 uppercase tracking-widest sm:tracking-[0.2em] truncate">
            {label}
          </p>
          <div className="flex items-baseline gap-1">
            ...
            <span className="text-emerald-500 font-black text-[10px] animate-pulse">
              ●
            </span>
          </div>
          <p className="text-[9px] sm:text-[10px] font-bold text-slate-500 truncate mt-1">
            {description}
          </p>
        </div>
      </div>
...
function FilterButton({
  ...
  return (
    <button
      onClick={onClick}
      className={`flex items-center gap-2 px-6 py-2.5 rounded-[1.2rem] font-black text-[10px] uppercase tracking-widest transition-all duration-300 ${
        active
    ...
    >
      {icon && (
        <span className={active ? "text-primary-foreground" : "text-slate-500"}>
          {icon}
        </span>
      )}
      {label}
