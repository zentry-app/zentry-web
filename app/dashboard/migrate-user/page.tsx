...
import { useAdminRequired } from '@/lib/hooks/useAdminRequired';
...
export default function MigrateUserPage() {
  // Proteger la página - solo global admins
  const { isUserLoading: isCheckingAuth, isGlobalAdmin } = useAdminRequired(true);
  const { user } = useAuth();
  ...
  // Si no es global admin después de cargar, el hook redirigirá automáticamente
  // Mostramos loading mientras redirige
  if (!isGlobalAdmin) {
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <Loader2 className="w-8 h-8 animate-spin text-gray-400" />
        <span className="ml-3 text-gray-500">Redirigiendo...</span>
      </div>
    );
  }
  ...
  return (
    <div className="container mx-auto py-8 px-4 max-w-6xl">
            ...
            {sourceValidation.user?.uid === destValidation.user?.uid ? (
              ...
              <>
                ...
                <div className="flex items-start space-x-3">
                  ...
                  />
                  <Label htmlFor="confirm" className="text-sm text-amber-800 dark:text-amber-300 leading-tight cursor-pointer">
      ...
      {migrationResult && (
        <Card className={migrationResult.success ? "border-green-200 bg-green-50 dark:bg-green-950/20" : "border-red-200 bg-red-50 dark:bg-red-950/20"}>
          <CardHeader>
            <CardTitle className={`flex items-center gap-2 ${migrationResult.success ? 'text-green-800 dark:text-green-400' : 'text-red-800 dark:text-red-400'}`}>
          ...
          <CardContent className={migrationResult.success ? 'text-green-700 dark:text-green-300' : 'text-red-700 dark:text-red-300'}>
            <div className="space-y-3">
              ...
              {migrationResult.errors.length > 0 && (
                <div className="mt-3 p-3 bg-yellow-100 dark:bg-yellow-900/20 rounded-lg">
                  <h4 className="font-medium text-yellow-800 dark:text-yellow-400">Advertencias:</h4>
      ...
      <Card className="bg-blue-50 dark:bg-blue-950/20 border-blue-200 mt-6">
        <CardHeader>
          <CardTitle className="text-blue-900 dark:text-blue-300">Instrucciones de Uso</CardTitle>
        </CardHeader>
        <CardContent className="text-blue-800 dark:text-blue-300 space-y-2 text-sm">
