...
const DetallesIngresoDialogContent: React.FC<
  DetallesIngresoDialogContentProps
> = ({ selectedIngreso, formatDateToFull }) => {
  const [usuarioGenerador, setUsuarioGenerador] = useState<Usuario | null>(
    null,
  );
  ...
  const { visitData, vehicleInfo, exitDetails, status, physicalPass } =
    selectedIngreso;
    ...
    <div className="flex flex-col h-full max-h-[90vh]">
          ...
          <div className="space-y-1">
            ...
            <div className="flex items-center gap-4 text-slate-300">
              ...
              <div className="flex items-center gap-1.5 text-xs font-bold uppercase tracking-widest">
                ...
                {selectedIngreso.entryMethod === "moroso_vehicular"
                  ? "MOROSO VEHICULAR"
                  : selectedIngreso.entryMethod === "moroso_pedestrian"
                    ? "MOROSO PEATONAL"
      ...
      <div className="flex-1 overflow-y-auto p-10 space-y-8 bg-slate-50/50 backdrop-blur-xl">
        ...
        {status === "completed" && (
          ...
          >
            {!exitDetails?.suspiciousCargo &&
            exitDetails?.passReturned !== false &&
            ...
            exitDetails?.exitInSameVehicle !== false ? (
              <div className="bg-emerald-500/10 border border-emerald-500/20 p-6 rounded-[2rem] flex items-start gap-4 ring-1 ring-emerald-500/5">
                ...
                <div>
                  <p className="text-emerald-800 font-black text-sm uppercase tracking-widest">
                    Acceso Seguro
                  </p>
                  <p className="text-emerald-600/80 text-sm font-bold mt-0.5">
                    El ciclo de visita se completó exitosamente sin incidencias
                    detectadas.
                  </p>
                </div>
              </div>
              ...
              <div className="bg-red-500/10 border border-red-500/20 p-6 rounded-[2rem] flex items-start gap-4 ring-1 ring-red-500/5">
                ...
                <div className="flex-1">
                  <p className="text-red-800 font-black text-sm uppercase tracking-widest">
                    Alerta de Seguridad
                  </p>
                  <ul className="mt-2 space-y-1">
                    {exitDetails?.suspiciousCargo && (
                      <li className="text-red-600 text-xs font-bold flex items-center gap-2">
                        <div className="h-1 w-1 bg-red-400 rounded-full" />{" "}
                        Carga sospechosa reportada
                      </li>
                    )}
                    {exitDetails?.passReturned === false && (
                      <li className="text-red-600 text-xs font-bold flex items-center gap-2">
                        <div className="h-1 w-1 bg-red-400 rounded-full" /> Pase
                        no devuelto
                      </li>
                    )}
                    {exitDetails?.samePersonExit === false && (
                      <li className="text-red-600 text-xs font-bold flex items-center gap-2">
                        <div className="h-1 w-1 bg-red-400 rounded-full" />{" "}
                        Discrepancia de identidad en salida
                      </li>
                    )}
                    {exitDetails?.exitInSameVehicle === false && (
                      <li className="text-red-600 text-xs font-bold flex items-center gap-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <ComparisonCard
            ...
            exitData={
              status === "completed"
                ? exitDetails?.samePersonExit === false
                  ? capitalizeName(exitDetails.differentPersonName || "Otro")
                  : "Cerrado"
                : "Pendiente"
            }
          ...
          <ComparisonCard
            ...
            entryData={
              vehicleInfo?.placa ? vehicleInfo.placa.toUpperCase() : "Peatonal"
            }
            subEntry={
              vehicleInfo?.placa
            ...
            exitData={
              status === "completed"
                ? exitDetails?.exitInSameVehicle === false
                  ? exitDetails.exitVehicleInfo?.placa || "Desconocido"
                  : "Cerrado"
                : "Pendiente"
            }
            ...
            <div className="space-y-4">
              <div>
                ...
                <p className="font-extrabold text-slate-800">
                  {selectedIngreso._residencialNombre}
                </p>
              </div>
              <div>
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
        ...
        {selectedIngreso.userId && (
          <Card className="border-none shadow-zentry bg-white rounded-[2.5rem] overflow-hidden">
            <div className="bg-slate-900 p-6 flex items-center justify-between">
              ...
              <Badge
                variant="outline"
                className="border-white/20 text-white font-black px-4 py-1 rounded-full text-[9px]"
              >
                GENERÓ CÓDIGO QR
              </Badge>
            </div>
              ...
              {loadingUsuario ? (
                  ...
                  <div className="flex-1 space-y-4">
                    <div>
                      ...
                      <p className="text-xl font-extrabold text-slate-900">
                        {capitalizeName(usuarioGenerador.fullName || "")}{" "}
                        {capitalizeName(
                          usuarioGenerador.paternalLastName || "",
                        )}
                      </p>
                    </div>
                    <div className="flex flex-wrap gap-4">
                      <div className="bg-slate-50 px-4 py-2 rounded-xl border border-slate-100 flex items-center gap-2">
                        ...
                        <span className="text-xs font-bold text-slate-600">
                          {usuarioGenerador.email}
                        </span>
                      </div>
                      <div className="bg-slate-50 px-4 py-2 rounded-xl border border-slate-100 flex items-center gap-2">
                        <Shield className="h-4 w-4 text-slate-400" />
                        <span className="text-xs font-bold text-slate-600 capitalize">
                          {usuarioGenerador.role === "resident"
                            ? "Residente"
                            : usuarioGenerador.role}
                        </span>
                      </div>
                    </div>
                  </div>
                ...
                <div className="text-center py-4 flex flex-col items-center gap-2">
                  ...
                  <p className="text-slate-400 font-bold text-sm">
                    El usuario generador ya no se encuentra en el sistema.
                  </p>
                </div>
              )}
            </CardContent>
...
function ComparisonCard({
  ...
  return (
    <Card
      className={`border-none shadow-zentry bg-white rounded-[2rem] overflow-hidden ring-1 ${isMismatch ? "ring-red-500/30" : "ring-slate-100"}`}
    >
      <div
        className={`p-4 flex items-center gap-2 ${isMismatch ? "bg-red-50" : "bg-slate-50"}`}
      >
        {icon}
        <p
          className={`text-[10px] font-black uppercase tracking-widest ${isMismatch ? "text-red-700" : "text-slate-500"}`}
        >
          {title}
        </p>
      </div>
      <div className="p-6 grid grid-cols-2 gap-4 divide-x divide-slate-100">
        ...
        <div className="pl-4 space-y-1">
          ...
          <p
            className={`font-extrabold leading-tight ${isMismatch ? "text-red-600" : "text-slate-900 opacity-60"}`}
          >
            {exitData}
          </p>
        </div>
