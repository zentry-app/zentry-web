...
export function ReservationAuditHistory({
  ...
  const loadAuditHistory = useCallback(async () => {
    if (!reservationId) {
      console.log("❌ [AUDIT] Falta reservationId:", {
        reservationId,
        residencialId,
      });
      return;
    ...
    console.log("🔍 [AUDIT] Cargando historial para:", {
      reservationId,
      residencialId,
    });
    console.log(
      "🔍 [AUDIT] Verificando si residencialId es global:",
    ...
    try {
      ...
      if (residencialId === "global" || !residencialId) {
        ...
        for (const residencialDoc of residencialesDocs) {
          ...
          if (reservationSnap.exists()) {
            ...
            console.log(
              "✅ [AUDIT] Reserva encontrada en residencial:",
              actualResidencialId,
            );
            break;
          }
        ...
        if (actualResidencialId === "global" || !actualResidencialId) {
          console.error(
            "❌ [AUDIT] No se pudo encontrar la reserva en ningún residencial",
          );
          setEvents([]);
          return;
        }
      ...
      console.log(
        "📍 [AUDIT] Ruta de consulta:",
        `residenciales/${actualResidencialId}/reservaciones/${reservationId}/historial`,
      );
      ...
      const auditEvents: AuditEvent[] = snapshot.docs.map((doc) => {
        const data = doc.data();
        console.log("📄 [AUDIT] Evento encontrado:", {
          id: doc.id,
  ...
  const generateRetroactiveHistory = async () => {
    ...
    try {
      ...
      if (residencialId === "global" || !residencialId) {
        console.log(
          "🔍 [AUDIT] Buscando residencial real para generar historial...",
        );

        const residencialesDocs2 = await getCachedResidenciales();

        for (const residencialDoc of residencialesDocs2) {
        ...
        if (actualResidencialId === "global" || !actualResidencialId) {
          console.error(
            "❌ [AUDIT] No se pudo encontrar la reserva para generar historial",
          );
          return;
        }
      }
      ...
      if (
        reservationData.status === "aprobada" ||
        reservationData.status === "en_curso" ||
        reservationData.status === "finalizado"
      ) {
        const approvedEvent = {
          reservationId: reservationId,
          residencialId: actualResidencialId,
      ...
      if (
        reservationData.estadoLlaves === "entregadas" ||
        reservationData.estadoLlaves === "recibidas"
      ) {
        const deliveredEvent = {
          ...
          timestamp:
            reservationData.fechaEntregaLlaves ||
            reservationData.createdAt ||
            serverTimestamp(),
      ...
      if (reservationData.estadoLlaves === "recibidas") {
        const receivedEvent = {
          ...
          timestamp:
            reservationData.fechaRecepcionLlaves ||
            reservationData.createdAt ||
            serverTimestamp(),
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-2xl max-h-[80vh]">
        <DialogHeader>
          ...
          <DialogDescription>
            Registro cronológico de todos los eventos relacionados con esta
            reserva
          </DialogDescription>
        </DialogHeader>
