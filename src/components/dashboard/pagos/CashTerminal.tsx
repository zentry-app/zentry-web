...
import {
  DollarSign, Loader2, CheckCircle, Printer, ShieldCheck, Search, Plus,
  Lock, Fingerprint, Clock, Send, Bell,
} from "lucide-react";
...
export default function CashTerminal({ open, onClose, residencialId, houses, onPaymentRegistered }: CashTerminalProps) {
  ...
  const [receipt, setReceipt] = useState<{ folio: string; house: string; resident: string; amount: string; timestamp: string; concept: string } | null>(null);
  ...
  useEffect(() => {
    ...
    getDocs(query(collection(db, `residenciales/${residencialId}/productos`), orderBy("name")))
  ...
  const handleSelectHouse = (h: HouseStatus) => { setSelectedHouse(h); setHouseSearch(h.label); };
  ...
  const handleSubmit = async () => {
    ...
    try {
      const concept = paymentType === "product" && selectedProduct ? selectedProduct.name : "Cuota mensual";
      const res = await httpsCallable<any, any>(functions, "apiRegisterDirectCashPayment")({
        ...
        data: {
          ...
          paymentType,
          productId: paymentType === "product" && selectedProduct ? selectedProduct.id : undefined,
          productName: paymentType === "product" && selectedProduct ? selectedProduct.name : undefined,
          concept,
  ...
  const handleNewPayment = () => {
    setReceipt(null); setSelectedHouse(null); setHouseSearch(""); setAmount(""); setNotes("");
    setPaymentType("fee"); setSelectedProduct(null);
    setState("form"); setProcessingStep(0);
  };
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => { if (!o && state !== "processing") handleClose(); }}>
        ...
        {state === "processing" && (
          <div className="p-10 text-center">
            ...
            <div className="relative h-24 w-24 mx-auto mb-8">
              <div className="absolute inset-0 rounded-full border-4 border-emerald-100 animate-ping" style={{ animationDuration: "1.5s" }} />
              <div className="absolute inset-2 rounded-full border-4 border-emerald-200 animate-ping" style={{ animationDuration: "2s" }} />
            ...
            <h3 className="text-lg font-black text-slate-900 mb-1">Procesando pago seguro</h3>
            <p className="text-xs text-muted-foreground mb-8">No cierres esta ventana</p>
            ...
            <div className="space-y-3 text-left max-w-[260px] mx-auto">
              {processingSteps.map((step, i) => {
                ...
                return (
                  <div key={i} className={`flex items-center gap-3 transition-all duration-300 ${isDone ? "opacity-50" : isActive ? "opacity-100" : "opacity-20"}`}>
                    <div className={`h-8 w-8 rounded-lg flex items-center justify-center shrink-0 transition-all duration-300 ${
                      isDone ? "bg-emerald-100" : isActive ? "bg-emerald-500 shadow-md shadow-emerald-500/30" : "bg-slate-100"
                    }`}>
                      {isDone ? <CheckCircle className="h-4 w-4 text-emerald-600" /> :
                        isActive ? <step.icon className="h-4 w-4 text-white animate-pulse" /> :
                        <step.icon className="h-4 w-4 text-slate-300" />}
                    </div>
                    <span className={`text-sm font-bold ${isDone ? "text-emerald-600" : isActive ? "text-slate-900" : "text-slate-300"}`}>
                      {step.label}
          ...
          <div className="p-8 animate-fadeIn">
            ...
            <div className="text-center mb-6">
              <div className="relative h-20 w-20 mx-auto mb-4">
                <div className="absolute inset-0 rounded-full bg-emerald-100 animate-ping" style={{ animationDuration: "1s", animationIterationCount: "2" }} />
              ...
              <h3 className="text-xl font-black text-slate-900">Pago registrado</h3>
            ...
            <div className="mt-4 flex items-center gap-2.5 p-3 bg-blue-50 border border-blue-200 rounded-xl">
              ...
              <div>
                <p className="text-xs font-bold text-blue-800">Recibo enviado automáticamente</p>
                <p className="text-[10px] text-blue-600">El residente recibirá una notificación con el comprobante</p>
              </div>
            </div>
                ...
                <div>
                  ...
                  <div className="flex items-center gap-3 mt-0.5">
                    ...
                    <span className="flex items-center gap-1 text-[10px] text-muted-foreground">
                      <Lock className="h-2.5 w-2.5" />Cifrado E2E
                    </span>
                    <span className="flex items-center gap-1 text-[10px] text-muted-foreground">
                      <Fingerprint className="h-2.5 w-2.5" />Auditable
                    </span>
                  </div>
                </div>
              </div>
            ...
            <div className="p-6 space-y-5">
              ...
              <div>
                ...
                <div className="relative mt-1.5">
                  ...
                  <Input placeholder="Buscar por casa o residente..." value={houseSearch}
                ...
                {selectedHouse && (
                  <div className="mt-2 flex items-center gap-2 px-3 py-2 bg-emerald-50 rounded-xl border border-emerald-200">
                    ...
                    {selectedHouse.deudaCents > 0 && <span className="text-[10px] font-bold text-red-500 shrink-0">Deuda: {fmtFull(selectedHouse.deudaCents)}</span>}
                  </div>
                )}
              </div>
              ...
              <div>
                <label className="text-[11px] font-bold text-slate-500 uppercase tracking-wide">¿Qué estás cobrando?</label>
                <div className="flex gap-2 mt-1.5">
                  <button onClick={() => { setPaymentType("fee"); setSelectedProduct(null); setAmount(""); }}
                    className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-bold transition-all ${
                      paymentType === "fee" ? "bg-blue-600 text-white shadow-md" : "bg-slate-100 text-slate-500 hover:bg-slate-200"
                    }`}>
                    <DollarSign className="h-4 w-4" />Cuota mensual
                  </button>
                  <button onClick={() => { setPaymentType("product"); setAmount(""); }}
                    className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-bold transition-all ${
                      paymentType === "product" ? "bg-purple-600 text-white shadow-md" : "bg-slate-100 text-slate-500 hover:bg-slate-200"
                    }`}>
                    <Tag className="h-4 w-4" />Producto
                  </button>
                </div>
                ...
                {paymentType === "product" && (
                  <div className="mt-2">
                    {loadingProducts ? (
                      ...
                      <p className="text-xs text-muted-foreground py-2">No hay productos configurados</p>
                    ) : (
                      <div className="space-y-1.5 max-h-[150px] overflow-y-auto">
                        {products.map(p => (
                          <button key={p.id} onClick={() => { setSelectedProduct(p); setAmount(String(p.priceCents / 100)); }}
                            className={`w-full flex items-center justify-between p-2.5 rounded-lg text-left transition-all ${
                              selectedProduct?.id === p.id ? "bg-purple-50 border-2 border-purple-300" : "bg-slate-50 border border-slate-200 hover:border-slate-300"
                            }`}>
                            <div>
                            ...
                            <span className="font-black text-xs text-purple-600">{fmtFull(p.priceCents)}</span>
                          </button>
                        ))}
                      </div>
                    )}
              ...
              <div>
                ...
                <Input placeholder="Notas o referencia del pago" value={notes} onChange={e => setNotes(e.target.value)}
                  className="mt-1.5 h-11 rounded-xl border-slate-200 font-medium text-sm" />
              </div>

              {/* Submit */}
              <Button className="w-full h-14 rounded-2xl font-black text-base bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 shadow-lg shadow-emerald-500/20 transition-all"
                onClick={handleSubmit} disabled={!selectedHouse || !amount}>
                <Lock className="h-5 w-5 mr-2" />Registrar pago seguro
              </Button>

              {/* Trust badges */}
              <div className="flex items-center justify-center gap-4 pt-1">
                <div className="flex items-center gap-1 text-[9px] text-muted-foreground"><ShieldCheck className="h-3 w-3 text-emerald-500" /><span>Folio automático</span></div>
                <div className="flex items-center gap-1 text-[9px] text-muted-foreground"><Fingerprint className="h-3 w-3 text-blue-500" /><span>Registro auditable</span></div>
                <div className="flex items-center gap-1 text-[9px] text-muted-foreground"><Send className="h-3 w-3 text-slate-400" /><span>Recibo automático</span></div>
              </div>
            </div>
          </>
        )}
