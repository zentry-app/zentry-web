...
export function EstadoCuenta({ residencialId, houseId }: EstadoCuentaProps) {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, (snap) => {
      const data = snap.docs.map(
        (d) => ({ id: d.id, ...d.data() }) as LedgerEntry,
      );
      setEntries(data);
      setLoading(false);
    });
  ...
  const charges = entries.filter(
    (e) =>
      e.impact === "INCREASE_DEBT" &&
      e.status === "applied" &&
      e.type !== "REVERSAL",
  );
  const payments = entries.filter(
    (e) => e.impact === "DECREASE_DEBT" && e.status === "applied",
  );
  const reversals = entries.filter((e) => e.type === "REVERSAL");
  const reversedPayments = entries.filter(
    (e) =>
      e.impact === "DECREASE_DEBT" &&
      e.status === "applied" &&
      reversals.some((r) => r.id === `REV_${e.id}`),
  );

  const totalCharged = charges.reduce(
    (s, e) => s + (e.amountInCents ?? e.amount),
    0,
  );
  const totalPaid = payments
    .filter((p) => !reversedPayments.includes(p))
    .reduce((s, e) => s + (e.amountInCents ?? e.amount), 0);

  const morosidadInfo =
    MOROSIDAD_CONFIG[balance?.estadoMorosidad ?? "al_dia"] ??
      ...
      <div className="rounded-2xl bg-gradient-to-br from-slate-900 to-slate-800 text-white p-6">
        ...
        <div className="grid grid-cols-2 gap-4 mt-6">
          ...
          <div className="bg-white/5 rounded-xl p-4">
            ...
            <p
              className={`text-2xl font-black ${(balance?.deudaAcumulada ?? 0) > 0 ? "text-red-400" : "text-emerald-400"}`}
            >
