...
import {
  DollarSign, Clock, CheckCircle, XCircle, RefreshCw, Banknote, Eye,
  TrendingUp, AlertTriangle, Home, Loader2, Image as ImageIcon,
  BookOpen, ArrowLeftRight, Tag, Settings, Search, LayoutGrid, List,
  ShieldCheck,
} from "lucide-react";
...
const CatalogManagement = dynamic(() => import("./CatalogManagement"), { ssr: false, loading: () => <p className="py-12 text-center text-muted-foreground">Cargando...</p> });
const BillingSettings = dynamic(() => import("./BillingSettings"), { ssr: false, loading: () => <p className="py-12 text-center text-muted-foreground">Cargando...</p> });
const MovementsManager = dynamic(() => import("./MovementsManager"), { ssr: false, loading: () => <p className="py-12 text-center text-muted-foreground">Cargando...</p> });
...
// ─── Component ────────────────────────────────────────────────────
export default function PaymentsDashboard({ residencialId }: { residencialId: string }) {
  ...
  const [selectedPayment, setSelectedPayment] = useState<PendingValidation | null>(null);
  ...
  const fetchAllData = useCallback(async () => {
    try {
      const [kpiRes, pendingRes, housesRes] = await Promise.allSettled([
        httpsCallable<any, KPIData>(functions, "getReportingDashboard")({ residencialId }),
        httpsCallable<any, PendingValidation[]>(functions, "getPendingValidationsList")({ residencialId }),
        httpsCallable<any, HouseStatus[]>(functions, "getResidentialHousesSummary")({ residencialId }),
      ]);
      ...
      setLoading(false); setLoadingHouses(false); setRefreshing(false);
    }
  }, [residencialId, refreshKey]);

  useEffect(() => {
    if (refreshKey === 0) { setLoading(true); setLoadingHouses(true); }
    fetchAllData();
  }, [fetchAllData]);

  const handleRefresh = () => { setRefreshing(true); triggerRefresh(); };

  const handleValidate = async (paymentId: string) => {
    setValidating(true);
    try {
      const res = await httpsCallable<any, any>(functions, "apiValidatePayment")({ residencialId, paymentId });
  ...
  const handleReject = async (paymentId: string) => {
    ...
    setRejecting(true);
    try {
      await httpsCallable(functions, "apiRejectPayment")({ residencialId, paymentId, reason: rejectReason.trim() });
  ...
  const kpiCards = [
    { label: "Cobrado mes", value: kpi ? fmtFull(kpi.summary.cobradoNetoCents) : "—", icon: TrendingUp, grad: "from-blue-500 to-blue-600" },
    { label: "Por validar", value: kpi ? String(kpi.validations.pendingValidation) : String(pending.length), icon: Clock, grad: "from-amber-500 to-orange-500", pulse: pending.length > 0 },
    { label: "Deuda total", value: houses.length > 0 ? fmtFull(localDebt) : "—", icon: AlertTriangle, grad: "from-red-500 to-rose-500" },
    { label: "Con deuda", value: houses.length > 0 ? `${localWithDebt}/${houses.length}` : "—", icon: Home, grad: "from-emerald-500 to-teal-500" },
  ];
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="text-3xl font-black tracking-tight text-gradient-zentry">Pagos</h2>
          <p className="text-muted-foreground font-medium text-sm">Panel de cobros y validaciones</p>
        </div>
        <div className="flex items-center gap-2">
          <Button variant="outline" size="sm" onClick={handleRefresh} disabled={refreshing} className="rounded-2xl hover-lift">
            <RefreshCw className={`h-4 w-4 mr-1.5 ${refreshing ? "animate-spin" : ""}`} />
            Actualizar
          </Button>
          <Button size="sm" onClick={() => setCashOpen(true)} className="rounded-2xl shadow-zentry bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 hover-lift font-bold">
        ...
        <Card className="border-none shadow-zentry-lg bg-white/80 backdrop-blur-2xl rounded-[2rem] overflow-hidden animate-fadeIn">
          <CardHeader className="px-6 pt-5 pb-2">
            <CardTitle className="text-sm font-black flex items-center gap-2">
              ...
              Pagos por validar
              {pending.length > 0 && <Badge className="bg-amber-100 text-amber-700 font-black rounded-full ml-auto">{pending.length}</Badge>}
            </CardTitle>
          </CardHeader>
          <CardContent className="px-6 pb-5">
            {loading ? (
              <div className="grid gap-2 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">{Array.from({ length: 6 }).map((_, i) => <Skeleton key={i} className="h-14 rounded-xl" />)}</div>
            ) : pending.length === 0 ? (
                  ...
                  <div key={p.id} onClick={() => setSelectedPayment(p)}
                    ...
                    <div className="min-w-0">
                      <div className="flex items-center gap-1.5">
                        <span className="font-bold text-sm">{p.houseLabel || p.houseId}</span>
                        <span className="text-[10px] text-muted-foreground px-1.5 py-0.5 bg-slate-100 rounded">{p.method === "transfer" ? "Transf." : "Efectivo"}</span>
                      </div>
                      <p className="text-xs text-muted-foreground truncate mt-0.5">{p.residentName}</p>
                    </div>
                    <div className="flex items-center gap-2 ml-3 shrink-0">
                      <span className="font-black text-sm">{fmtFull(p.montoCents)}</span>
      ...
      {activeTab === "estado_cuenta" && (
        <div className="animate-fadeIn"><EstadoCuentaView houses={houses} loadingHouses={loadingHouses} counts={counts} onSelectHouse={setSelectedHouse} /></div>
      )}

      {/* ─── Tab: Libro Mayor ────────────────────────────────────── */}
      {activeTab === "libro_mayor" && <div className="animate-fadeIn"><AccountingDashboard residencialId={residencialId} /></div>}
      ...
      {/* ─── Cash Terminal ───────────────────────────────────────── */}
      {cashOpen && (
        <CashTerminal open={cashOpen} onClose={() => setCashOpen(false)} residencialId={residencialId} houses={houses} onPaymentRegistered={() => triggerRefresh(1200)} />
      )}

      {/* ─── House Detail Sheet ──────────────────────────────────── */}
      {selectedHouse && (
        <HouseDetailSheet open={!!selectedHouse} onClose={() => setSelectedHouse(null)} residencialId={residencialId} house={selectedHouse} onAction={() => triggerRefresh(1200)} />
      )}

      {/* ─── Validation Dialog ───────────────────────────────────── */}
      {selectedPayment && (
        <Dialog open={!!selectedPayment} onOpenChange={open => { if (!open) { setSelectedPayment(null); setRejectReason(""); } }}>
          <DialogContent className="max-w-lg rounded-[2rem] border-none shadow-2xl bg-white/95 backdrop-blur-3xl">
            <DialogHeader>
              <DialogTitle className="text-lg font-black">Validar comprobante</DialogTitle>
              <DialogDescription className="font-medium">{selectedPayment.houseLabel} · {selectedPayment.residentName}</DialogDescription>
            </DialogHeader>
            <div className="space-y-4">
              <div className="text-center py-3 bg-gradient-to-br from-primary/5 to-primary/10 rounded-2xl">
                <div className="text-2xl font-black">{fmtFull(selectedPayment.montoCents)}</div>
                <div className="flex items-center justify-center gap-2 mt-1.5">
                  <Badge variant="outline" className="rounded-lg font-bold text-[11px]">{selectedPayment.method === "transfer" ? "Transferencia" : "Efectivo"}</Badge>
                  {selectedPayment.fecha && <span className="text-[11px] text-muted-foreground">{new Date(selectedPayment.fecha).toLocaleDateString("es-MX", { day: "numeric", month: "short", year: "numeric" })}</span>}
                </div>
              </div>
              {selectedPayment.notas && (
                <div className="bg-slate-50 rounded-xl p-3">
                  <span className="text-[10px] font-bold text-muted-foreground uppercase">Notas</span>
              ...
              {selectedPayment.proofUrl && (
                <a href={selectedPayment.proofUrl} target="_blank" rel="noopener noreferrer">
                  <div className="relative rounded-xl overflow-hidden border hover:shadow-zentry transition-all group">
                    <img src={selectedPayment.proofUrl} alt="Comprobante" className="w-full h-44 object-cover" />
                    <div className="absolute inset-0 bg-black/0 group-hover:bg-black/10 transition-all flex items-center justify-center">
                      <span className="bg-white/90 backdrop-blur rounded-lg px-2.5 py-1 opacity-0 group-hover:opacity-100 text-xs font-bold flex items-center gap-1"><ImageIcon className="h-3 w-3" />Ver</span>
                    </div>
                  </div>
                </a>
              )}
              <div>
                <label className="text-[10px] font-bold text-muted-foreground uppercase">Motivo de rechazo</label>
                <Input placeholder="Solo si rechazas" value={rejectReason} onChange={e => setRejectReason(e.target.value)} className="mt-1 h-10 rounded-xl" />
              </div>
              <div className="flex gap-3">
                <Button className="flex-1 h-11 rounded-xl font-black bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600"
                  onClick={() => handleValidate(selectedPayment.id)} disabled={validating || rejecting}>
                  {validating ? <Loader2 className="h-4 w-4 mr-2 animate-spin" /> : <CheckCircle className="h-4 w-4 mr-2" />}Validar
                </Button>
                <Button className="flex-1 h-11 rounded-xl font-black" variant="destructive"
                  onClick={() => handleReject(selectedPayment.id)} disabled={validating || rejecting}>
                  {rejecting ? <Loader2 className="h-4 w-4 mr-2 animate-spin" /> : <XCircle className="h-4 w-4 mr-2" />}Rechazar
                </Button>
              </div>
            </div>
          </DialogContent>
        ...
        <div className="flex flex-wrap items-center gap-3">
            ...
            {[
              ...
              <button key={f.key} onClick={() => setFilter(f.key)}
                className={`flex items-center gap-1.5 text-xs font-bold px-3 py-1.5 rounded-xl transition-all ${
                  filter === f.key ? (f.active || "text-slate-800 bg-slate-100") + " shadow-sm" : "text-slate-400 hover:text-slate-600"
                }`}>
                <span className={`h-2 w-2 rounded-full ${f.dot}`} />{f.label} ({f.count})
              </button>
            ))}
          </div>
          <div className="flex items-center gap-2 ml-auto">
            <div className="relative">
              ...
              <Input placeholder="Buscar..." value={search} onChange={e => setSearch(e.target.value)} className="pl-9 h-9 w-48 text-sm rounded-xl border-slate-200" />
            </div>
            <div className="flex items-center border border-slate-200 rounded-lg overflow-hidden">
              <button onClick={() => setViewMode("list")} className={`p-2 transition-colors ${viewMode === "list" ? "bg-slate-900 text-white" : "text-slate-400"}`}><List className="h-4 w-4" /></button>
              <button onClick={() => setViewMode("grid")} className={`p-2 transition-colors ${viewMode === "grid" ? "bg-slate-900 text-white" : "text-slate-400"}`}><LayoutGrid className="h-4 w-4" /></button>
            </div>
          </div>
        </div>
      </CardHeader>
        ...
        {loadingHouses ? (
          ...
          <ScrollArea className="h-[500px]">
            <table className="w-full">
              <thead><tr className="text-[11px] font-bold text-muted-foreground uppercase tracking-wider border-b sticky top-0 bg-white/90 backdrop-blur">
                <th className="text-left py-2.5 pl-2 w-8"></th><th className="text-left py-2.5">Casa</th><th className="text-left py-2.5">Residente</th><th className="text-left py-2.5">Estado</th><th className="text-right py-2.5">Deuda</th><th className="text-right py-2.5 pr-2">A favor</th>
              </tr></thead>
              <tbody>
                {filtered.map(h => (
            ...
            {filtered.length === 0 && <p className="text-center text-muted-foreground py-12 text-sm">Sin resultados</p>}
          </ScrollArea>
        ) : (
          <ScrollArea className="h-[500px]">
