    ...
    try {
      ...
      const rows: HR[] = houses.map((h) => {
        ...
        return {
          ...
          pagadoCents: pagado,
          fechaPago: fechaMap[h.houseId] ?? null,
          mesesCubiertos: meses,
          cubreMes: meses.includes(reportMonth),
      ...
      rows.forEach((h) => {
        ...
        if (h.mesesCubiertos.includes(reportMonth)) {
          ...
          paraOtrosMesesCents += h.pagadoCents;
        }
      });
      ...
      const styleD = (row: any, even: boolean) =>
        row.eachCell((cell: any) => {
          cell.fill = {
            type: "pattern",
            pattern: "solid",
            fgColor: { argb: argb(even ? C.grisHeader : C.blanco) },
          };
      ...
      Object.assign(wsR.getCell("B2"), {
        value: `REPORTE DE COBRANZA — ${mesNombre}`,
      ...
      Object.assign(wsR.getCell("B3"), {
        value: `Generado el ${new Date().toLocaleDateString("es-MX", { day: "2-digit", month: "long", year: "numeric" })}`,
      ...
      styleH(kL, C.azulOscuro);
      kL.height = 22;
      const kV = wsR.getRow(6);
      kV.values = [
        "",
        totalCasas,
        fmtMXN(totalRecaudado),
        fmtMXN(totalDeuda),
        `${casasQuePagaron}/${totalCasas}`,
        `${pctCobranza}%`,
      ];
      ...
      Object.assign(wsR.getCell("B8"), {
        value: "DESGLOSE DEL RECAUDADO",
      ...
      [
        [
          "Para cuota de " + mesCorto(reportMonth),
          fmtMXN(paraMesActualCents),
          "Pagos que liquidan la cuota del mes",
        ],
        [
          "Para meses anteriores",
          fmtMXN(paraOtrosMesesCents),
          "Catch-up de adeudos pasados",
        ],
        [
          "Total recaudado",
          fmtMXN(totalRecaudado),
        ...
        const bg =
          i === 2 ? C.azulClaro : i % 2 === 0 ? C.blanco : C.grisHeader;
        [2, 3, 4, 5].forEach((c) => {
          row.getCell(c).fill = {
            type: "pattern",
            pattern: "solid",
      ...
      // ── HOJA 2: PAGARON ──
      const wsP = wb.addWorksheet("Pagaron", {
      ...
      Object.assign(wsP.getCell("B2"), {
        value: `CASAS QUE PAGARON — ${mesNombre}`,
      ...
      Object.assign(wsP.getCell("B3"), {
        value: `${casasQuePagaron} casas  ·  Total: ${fmtMXN(totalRecaudado)}`,
        font: { size: 10, color: { argb: argb(C.blanco) }, italic: true },
        fill: {
          type: "pattern",
          pattern: "solid",
      ...
      // ── HOJA 3: PENDIENTES ──
      const wsPe = wb.addWorksheet("Pendientes", {
      ...
      Object.assign(wsPe.getCell("B2"), {
        value: `PENDIENTES DE PAGO — ${mesNombre}`,
        font: { bold: true, size: 14, color: { argb: argb(C.blanco) } },
        fill: {
          type: "pattern",
          pattern: "solid",
      ...
      Object.assign(wsPe.getCell("B3"), {
        value: `${rows.length - casasQuePagaron} casas sin pago  ·  Deuda total: ${fmtMXN(totalDeuda)}`,
        font: { size: 10, color: { argb: argb(C.blanco) }, italic: true },
        fill: {
          type: "pattern",
          pattern: "solid",
      ...
      wsPe.getRow(3).height = 18;
      wsPe.getRow(4).height = 8;
      ...
      const wsA = wb.addWorksheet("Estado Completo", {
      ...
      Object.assign(wsA.getCell("B2"), {
        value: `ESTADO COMPLETO — ${mesNombre}`,
        font: { bold: true, size: 14, color: { argb: argb(C.blanco) } },
        fill: {
          type: "pattern",
          pattern: "solid",
      ...
      wsA.getRow(2).height = 35;
      wsA.getRow(3).height = 8;
      ...
      aH.values = [
        ...
        "Deuda ($)",
        "A Favor ($)",
        `Pagado ${mesCorto(reportMonth)} ($)`,
        "Fecha de pago",
        "Meses cubiertos",
      ];
      styleH(aH, C.azulOscuro);
      aH.height = 22;
