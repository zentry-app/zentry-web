...
const FeatureCarousel = () => {
  ...
  const features = [
    ...
    },
    {
      ...
      description:
        "El sistema registra automáticamente las placas del vehículo.",
  ...
  useEffect(() => {
    ...
    const onMouseMove = (e: MouseEvent) => {
      if (!cursorFollowerRef.current || !arrowLeftRef.current || !arrowRightRef.current) return;
    ...
    const onClick = (e: MouseEvent) => {
      const rect = carousel.getBoundingClientRect();
      const x = e.clientX - rect.left;
      if (x < rect.width / 2 && currentIndex > 0) prevSlide();
      else if (x >= rect.width / 2 && currentIndex < maxIndex) nextSlide();
    };
    ...
    return () => {
      ...
      carousel.removeEventListener("click", onClick);
    };
      ...
      <div className="container mx-auto" ref={containerRef}>
        <div className="text-center mb-16 px-4">
          <h2 className="font-heading font-bold text-4xl lg:text-5xl mb-4">
            Acceso seguro y eficiente con QR
          </h2>
          <p className="text-lg text-gray-600 max-w-2xl mx-auto">
            Descubre cómo Zentry simplifica el ingreso de visitantes en tu
            residencial
          </p>
        </div>
