...
import { getCachedResidenciales } from "@/lib/cache/firebase-cache";
  ...
  try {
    ...
    if (opciones?.getAll) {
      ...
      try {
        ...
        if (debug)
          console.log(
            `✅ Total de usuarios obtenidos: ${snapshot.docs.length}`,
          );
        ...
        if (debug)
          console.log("⚠️ Método simple falló, usando método alternativo...");
    ...
    if (debug)
      console.log(
        `🔍 Obteniendo usuarios con límite: ${limitValue}, ordenado por: ${orderByField}`,
      );
    ...
    if (debug)
      console.log(
        `✅ Usuarios obtenidos: ${snapshot.docs.length}/${limitValue}`,
      );
...
export const getUsuariosPorResidencial = async (
  residencialId: string,
  opciones?: {
  ...
  try {
    ...
    const debug = opciones?.debug || false;
    if (debug)
      console.log(`🔍 Obteniendo usuarios del residencial: ${residencialId}`);

    // Si getAll es true, usar la nueva función que obtiene todos los usuarios
    if (opciones?.getAll) {
      if (debug)
        console.log(
          "🔄 Usando método getAll para obtener todos los usuarios...",
        );
      ...
      try {
        ...
        if (debug)
          console.log(
            `✅ Usuarios del residencial ${residencialId}: ${usuariosFiltrados.length}`,
          );
        return usuariosFiltrados;
      } catch (error) {
        if (debug)
          console.log("⚠️ Método getAll falló, usando método alternativo...");
        ...
        if (debug)
          console.log(
            `✅ Usuarios del residencial ${residencialId} (método alternativo): ${usuariosFiltrados.length}`,
          );
        return usuariosFiltrados;
      }
    }
    ...
    try {
      ...
      console.log(
        `✅ Usuarios del residencial ${residencialId} (método de respaldo): ${usuariosFiltrados.length}`,
      );
      return usuariosFiltrados;
    } catch (backupError) {
      console.error("❌ Método de respaldo también falló:", backupError);
...
export const isRegistrationComplete = (usuario: Usuario): boolean => {
  ...
  const isOwner =
    usuario.ownershipStatus === "own" || (usuario as any).isOwner === true;
  const isTenant =
    usuario.ownershipStatus === "rent" || (usuario as any).isOwner === false;
  ...
  if (
    !usuario.ownershipStatus &&
    typeof (usuario as any).isOwner !== "boolean"
  ) {
  ...
  if (isOwner) {
    ...
    if (isComplete) {
      const propietarioTipo =
        tenantCodes.length > 0 ? "que renta" : "que vive en casa";
...
export const getUsuariosPendientes = async (opciones?: { limit?: number }) => {
  try {
    ...
    return usuariosPendientes.filter((usuario) =>
      isRegistrationComplete(usuario),
    );
  } catch (error) {
    throw error;
  }
...
export const getUsuariosPendientesIncompletos = async (opciones?: {
  limit?: number;
}) => {
  try {
    const usuariosRef = collection(db, "usuarios");
    ...
    return usuariosPendientes.filter(
      (usuario) => !isRegistrationComplete(usuario),
    );
  } catch (error) {
    throw error;
  }
...
export const findTenantInvitation = async (
  residencialId: string,
  code: string,
) => {
  try {
...
export const markTenantInvitationAsUsed = async (
  residencialId: string,
  code: string,
  userId: string,
) => {
...
export const deleteTenantInvitation = async (
  residencialId: string,
  code: string,
) => {
  try {
...
export const expireTenantInvitation = async (
  residencialId: string,
  code: string,
) => {
  try {
...
export const crearUsuario = async (
  usuarioData: Partial<Usuario> & { uid: string },
) => {
  try {
    if (!usuarioData.uid) {
    ...
    if (usuarioData.uid.length < 10) {
      throw new Error(
        "El UID proporcionado no parece válido (demasiado corto)",
      );
    }
    ...
    if (
      ...
      usuarioData.signInProvider === "manual_creation"
    ) {
      throw new Error(
        "No se puede crear un usuario con el mismo UID que el usuario actual",
      );
    }
  ...
  try {
    ...
    if (isMoroso) {
      ...
      if (usuarioActual.residencialID) {
        try {
          const residencial = await getResidencialPorID(
            usuarioActual.residencialID,
          );
          ...
          console.warn(
            `⚠️ No se pudo obtener la configuración del residencial, usando valor por defecto (5):`,
            error,
          );
        }
      }
      ...
      if (usuarioActual.max_codigos_qr_diarios !== undefined) {
        datosActualizacion.max_codigos_qr_original =
          usuarioActual.max_codigos_qr_diarios;
      }
    } else {
      ...
      if (usuarioActual.max_codigos_qr_original !== undefined) {
        datosActualizacion.max_codigos_qr_diarios =
          usuarioActual.max_codigos_qr_original;
    ...
    try {
      ...
      const houseId =
        usuarioActual.houseID ||
        (usuarioActual.domicilio
          ? `${usuarioActual.domicilio.calle}#${usuarioActual.domicilio.houseNumber}`
          : null);
      ...
      console.warn(
        `⚠️ Error escribiendo historial de morosidad (no crítico):`,
        historialError,
      );
    }
    ...
    console.error(
      `❌ Error al cambiar estado de moroso para usuario ${id}:`,
      error,
    );
    throw error;
  }
...
export const eliminarUsuario = async (id: string) => {
  try {
    ...
    try {
      ...
      console.log(
        `📝 Datos del usuario a eliminar: ${userData.email || "email desconocido"}, rol: ${userData.role || "rol desconocido"}`,
      );
      ...
      console.error(
        `❌ Error al acceder o eliminar el documento de usuario ${id}:`,
        error,
      );
      ...
      if (error.code === "permission-denied") {
        throw new Error(
          `No tienes permisos para eliminar el usuario con ID ${id}`,
        );
      }
    ...
    console.error(
      `❌ Error al eliminar usuario ${id}: [${errorCode}] ${errorMsg}`,
    );
...
export const crearResidencial = async (
  residencial: Omit<Residencial, "id" | "fechaRegistro" | "fechaActualizacion">,
) => {
  try {
    const residencialesRef = collection(db, "residenciales");
...
export const actualizarResidencial = async (
  id: string,
...
export const getAreasComunes = async (residencialID: string) => {
  try {
    const areasComunesRef = collection(
      db,
      `residenciales/${residencialID}/areasComunes`,
    );
...
export const getAreaComun = async (residencialID: string, areaId: string) => {
  try {
    const areaComunRef = doc(
      db,
      `residenciales/${residencialID}/areasComunes/${areaId}`,
    );
...
export const crearAreaComun = async (
  residencialID: string,
  areaComun: Omit<AreaComun, "id" | "fechaRegistro" | "fechaActualizacion">,
) => {
  try {
    const areasComunesRef = collection(
      db,
...
export const actualizarAreaComun = async (
  residencialID: string,
  areaId: string,
  areaComun: Partial<AreaComun>,
) => {
  try {
    const areaComunRef = doc(
      db,
...
export const eliminarAreaComun = async (
  residencialID: string,
  areaId: string,
) => {
  try {
...
export const suscribirseAAreasComunes = (
  residencialID: string,
  callback: (areasComunes: AreaComun[]) => void,
) => {
  try {
    const areasComunesRef = collection(
      db,
...
export const getGuardias = async (residencialID: string) => {
  try {
    ...
    const guardiasRef = collection(
      db,
      `residenciales/${residencialID}/guardias`,
    );
...
export const getGuardia = async (residencialID: string, guardiaID: string) => {
  try {
    ...
    const guardiaRef = doc(
      db,
      `residenciales/${residencialID}/guardias/${guardiaID}`,
    );
...
export const crearGuardia = async (
  residencialID: string,
  guardia: Omit<Guardia, "id" | "fechaRegistro" | "fechaActualizacion">,
) => {
  try {
    if (!residencialID) {
      throw new Error("ID del residencial es requerido");
...
export const actualizarGuardia = async (
  residencialID: string,
  guardiaID: string,
  guardia: Partial<Guardia>,
) => {
  try {
    if (!residencialID || !guardiaID) {
      throw new Error("ID del residencial y del guardia son requeridos");
...
export const eliminarGuardia = async (
  residencialID: string,
  guardiaID: string,
) => {
  try {
...
export const suscribirseAGuardias = (
  residencialID: string,
  callback: (guardias: Guardia[]) => void,
) => {
  try {
    if (!residencialID) {
      throw new Error("ID del residencial es requerido");
...
export const crearTag = async (
  tag: Omit<Tag, "id" | "fechaRegistro" | "fechaActualizacion">,
) => {
  try {
    const tagsRef = collection(db, "tags");
    const nuevoTag = {
...
export const crearEvento = async (
  evento: Omit<Evento, "id" | "fechaRegistro" | "fechaActualizacion">,
) => {
  try {
    const eventosRef = collection(db, "eventos");
    const nuevoEvento = {
...
export const suscribirseAEventos = (
  ...
  return onSnapshot(q, (snapshot) => {
    callback(snapshot.docs.map((d) => ({ id: d.id, ...d.data() }) as Evento));
  });
};

export const getAsistentesEvento = async (
  eventoId: string,
): Promise<AsistenteEvento[]> => {
  const snap = await getDocs(collection(db, `eventos/${eventoId}/asistentes`));
  return snap.docs.map(
    (d) => ({ userId: d.id, ...d.data() }) as AsistenteEvento,
  );
};
...
export const crearAlerta = async (
  alerta: Omit<Alerta, "id" | "fechaRegistro" | "fechaActualizacion">,
) => {
  try {
    const alertasRef = collection(db, "alertas");
    const nuevaAlerta = {
...
export const getAlertasPanico = async (residencialID: string) => {
  try {
    console.log(
      `🔍 getAlertasPanico: Buscando alertas para residencial ID: ${residencialID}`,
    );
    ...
    for (const nombreColeccion of coleccionesPosibles) {
      try {
        ...
        const coleccionRef = collection(
          db,
          `residenciales/${residencialID}/${nombreColeccion}`,
        );
        const snapshot = await getDocs(coleccionRef);
        console.log(
          `📊 Subcolección '${nombreColeccion}': ${snapshot.docs.length} documentos`,
        );
        ...
        if (snapshot.docs.length > 0) {
          ...
          console.log(
            `✅ Se encontraron ${alertasEncontradas.length} alertas en la subcolección '${nombreColeccion}'`,
          );
        ...
        console.error(
          `❌ Error al buscar en subcolección '${nombreColeccion}':`,
          error,
        );
    ...
    if (alertas.length === 0) {
      console.log(
        `🔍 No se encontraron alertas en subcolecciones. Buscando en colecciones raíz...`,
      );
      ...
      for (const nombreColeccion of coleccionesPosibles) {
        try {
          ...
          const snapshot = await getDocs(q);
          console.log(
            `📊 Colección raíz '${nombreColeccion}': ${snapshot.docs.length} documentos para residencial ${residencialID}`,
          );
          ...
          console.error(
            `❌ Error al buscar en colección raíz '${nombreColeccion}':`,
            error,
          );
          // Continuar con la siguiente colección
        }
    ...
    if (alertas.length === 0) {
      console.log(
        `🔍 Último intento: buscando todas las alertas sin filtrar por residencial...`,
      );
      ...
      for (const nombreColeccion of coleccionesPosibles) {
        try {
          ...
          const snapshot = await getDocs(coleccionRef);
          console.log(
            `📊 Colección raíz '${nombreColeccion}' (sin filtro): ${snapshot.docs.length} documentos`,
          );
          ...
          console.error(
            `❌ Error al buscar en colección raíz '${nombreColeccion}' (sin filtro):`,
            error,
          );
          // Continuar con la siguiente colección
        }
    ...
    if (alertas.length === 0) {
      console.warn(
        `⚠️ No se encontraron alertas para el residencial ${residencialID} en ninguna colección`,
      );
    } else {
...
export const getAlertasPanicoActivas = async (residencialID: string) => {
  try {
    console.log(
      `🔍 getAlertasPanicoActivas: Buscando alertas activas para residencial ID: ${residencialID}`,
    );
    ...
    const snapshot = await getDocs(q);
    console.log(
      `✅ Consulta completada. Alertas activas encontradas: ${snapshot.docs.length}`,
    );
...
export const actualizarEstadoAlertaPanico = async (
  residencialID: string,
  alertaID: string,
  estado: AlertaPanico["status"],
) => {
  try {
    console.log(
      `🔄 actualizarEstadoAlertaPanico: Actualizando alerta ${alertaID} a estado ${estado} en residencial ${residencialID}`,
    );
    ...
    const residenciales = residencialesSnapshot.docs.map((doc) => ({
      id: doc.id,
      ...doc.data(),
    })) as Residencial[];
    ...
    if (docSnap.exists()) {
      console.log(
        `✅ El ID ${residencialID} es un ID válido de documento residencial`,
      );
      residencialRealID = residencialID;
      encontradoMapeo = true;
    } else {
      console.log(
        `⚠️ El ID ${residencialID} no es un ID válido de documento residencial, buscando alternativas...`,
      );
      ...
      for (const residencial of residenciales) {
        ...
        if (
          residencial.residencialID === residencialID ||
          ...
          encontradoMapeo = true;
          console.log(
            `✅ Encontrado mapeo: código ${residencialID} -> ID real ${residencialRealID}`,
          );
          break;
        }
      }
      ...
      if (!encontradoMapeo) {
        console.log(
          `⚠️ No se encontró mapeo para código ${residencialID}, intentando encontrar alertas...`,
        );
        ...
        for (const residencial of residenciales) {
          ...
          console.log(
            `🔍 Verificando si la alerta existe en residencial ${residencial.nombre} (${residencialId})`,
          );

          // Verificar si la alerta existe en este residencial
          const alertaRef = doc(
            db,
            `residenciales/${residencialId}/alertas`,
            alertaID,
          );
          ...
          if (alertaSnap.exists()) {
            ...
            encontradoMapeo = true;
            console.log(
              `✅ ¡Alerta encontrada en residencial ${residencial.nombre} (${residencialId})!`,
            );
            break;
          }
        }
    ...
    if (!encontradoMapeo) {
      console.log(
        `🔍 Buscando dinámicamente en todos los residenciales para alerta: ${alertaID}`,
      );
      ...
      try {
        ...
        for (const residencialDoc of residencialesDocs) {
          ...
          if (alertaSnap.exists()) {
            ...
            encontradoMapeo = true;
            console.log(
              `✅ ¡Alerta encontrada en residencial dinámico: ${residencialId}!`,
            );
            break;
          }
        }
    ...
    console.log(
      `🔍 Resultado de mapeo: ID original ${residencialID} -> ID para búsqueda ${residencialRealID}`,
    );
    ...
    for (const nombreColeccion of coleccionesPosibles) {
      try {
        console.log(
          `🔍 Intentando actualizar en subcolección: ${nombreColeccion}`,
        );
        const alertaRef = doc(
          db,
          `residenciales/${residencialRealID}/${nombreColeccion}`,
          alertaID,
        );
        ...
        if (docSnap.exists()) {
          console.log(
            `✅ Documento encontrado en subcolección '${nombreColeccion}'`,
          );
          ...
          if (data.status !== undefined) {
            ...
            console.log(
              `🔤 Usando estructura en español (estado: ${estadoEspanol})`,
            );
            Object.assign(actualizacion, {
          ...
          console.log(
            `📝 Datos a actualizar: ${JSON.stringify(actualizacion)}`,
          );

          try {
            await updateDoc(alertaRef, actualizacion);
            console.log(
              `✅ Alerta actualizada correctamente en subcolección '${nombreColeccion}'`,
            );
            actualizado = true;
            console.log(
              `🎉 ÉXITO: Alerta ${alertaID} actualizada a estado ${estado} en ${alertaRef.path}`,
            );
          ...
          console.log(
            `⚠️ Documento no existe en subcolección '${nombreColeccion}'`,
          );
        }
      } catch (error) {
        console.error(
          `❌ Error al actualizar en subcolección '${nombreColeccion}':`,
          error,
        );
        // Continuar con la siguiente colección
      }
    ...
    if (!actualizado) {
      console.log(
        `🔍 No se encontró en subcolecciones. Intentando en colecciones raíz...`,
      );

      for (const nombreColeccion of coleccionesPosibles) {
        try {
          console.log(
            `🔍 Buscando alerta en colección raíz: ${nombreColeccion}`,
          );
          ...
          if (docSnap.exists()) {
            console.log(
              `✅ Documento encontrado en colección raíz '${nombreColeccion}'`,
            );
            console.log(`📄 Datos actuales: ${JSON.stringify(docSnap.data())}`);
            ...
            try {
              await updateDoc(alertaRef, actualizacion);
              console.log(
                `✅ Alerta actualizada correctamente en colección raíz '${nombreColeccion}'`,
              );
              actualizado = true;
              console.log(
                `🎉 ÉXITO: Alerta ${alertaID} actualizada a estado ${estado} en ${alertaRef.path}`,
            ...
            console.log(
              `⚠️ Documento no existe en colección raíz '${nombreColeccion}'`,
            );
          }
        } catch (error) {
          console.error(
            `❌ Error al actualizar en colección raíz '${nombreColeccion}':`,
            error,
          );
          // Continuar con la siguiente colección
        }
    ...
    if (!actualizado) {
      console.error(
        `❌ No se pudo encontrar la alerta ${alertaID} en ninguna colección`,
      );
      console.log(
        `📌 RESUMEN: Intentamos actualizar alerta ${alertaID} en residencial ${residencialID} (mapeado a ${residencialRealID}) sin éxito`,
      );
...
export const marcarAlertaPanicoComoLeida = async (
  residencialID: string,
  alertaID: string,
) => {
  try {
    console.log(
      `📖 marcarAlertaPanicoComoLeida: Marcando alerta ${alertaID} como leída`,
    );
    ...
    for (const nombreColeccion of coleccionesPosibles) {
      try {
        console.log(
          `🔍 Intentando marcar como leída en colección: ${nombreColeccion}`,
        );
        const alertaRef = doc(
          db,
          `residenciales/${residencialID}/${nombreColeccion}`,
        ...
        if (docSnap.exists()) {
          console.log(
            `✅ Documento encontrado en colección '${nombreColeccion}'`,
          );
          ...
          console.log(
            `✅ Alerta marcada como leída correctamente en colección '${nombreColeccion}'`,
          );
          actualizado = true;
          break;
        }
        ...
        console.error(
          `❌ Error al marcar como leída en colección '${nombreColeccion}':`,
          error,
        );
        // Continuar con la siguiente colección
      }
    ...
    if (!actualizado) {
      throw new Error(
        `No se pudo marcar la alerta como leída. No se encontró en ninguna colección.`,
      );
    }
  } catch (error) {
...
export const suscribirseAAlertasPanico = (
  residencialID: string,
  callback: (alertas: AlertaPanico[]) => void,
) => {
  try {
    let unsubscribe: () => void = () => {};
    console.log(
      `🔔 suscribirseAAlertasPanico: Iniciando suscripción para residencial ID: ${residencialID}`,
    );
    ...
    const probarColeccion = (
      nombreColeccion: string,
      esColeccionRaiz = false,
    ) => {
      try {
        ...
        console.log(
          `🔍 Suscripción: Intentando con ${esColeccionRaiz ? "colección raíz" : "subcolección"}: ${coleccionPath}`,
        );
        ...
        return onSnapshot(
          ...
          (snapshot) => {
            console.log(
              `📊 Suscripción: ${nombreColeccion} (${esColeccionRaiz ? "raíz" : "subcolección"}): ${snapshot.docs.length} documentos`,
            );
            ...
            if (snapshot.docs.length > 0) {
              ...
              console.log(
                `✅ Suscripción: Enviando ${alertas.length} alertas al componente`,
              );
              callback(alertas);
          ...
          (error) => {
            console.error(
              `❌ Error en suscripción a ${nombreColeccion}:`,
              error,
            );
        ...
        console.error(
          `❌ Error al iniciar suscripción a ${nombreColeccion}:`,
          error,
        );
    ...
    setTimeout(() => {
      console.log(
        "⏳ Verificando si se encontraron alertas en subcolecciones...",
      );
    ...
    return () => {
      console.log(
        `🛑 Cancelando todas las suscripciones para residencial: ${residencialID}`,
      );
...
export const suscribirseAUsuarios = (
  ...
  console.log(
    `🔧 Configurando suscripción a usuarios con límite: ${limitValue}`,
  );
  ...
  if (opciones?.residencialID) {
    ...
    console.log(
      `🔧 Suscripción filtrada por residencial: ${opciones.residencialID}`,
    );
  } else {
    q = query(
      collection(db, "usuarios"),
...
export const suscribirseAUsuariosPendientes = (
  ...
  return onSnapshot(
    ...
    (snapshot) => {
      ...
      // Filtrar solo usuarios con registro completo
      const usuariosCompletos = usuarios.filter((usuario) =>
        isRegistrationComplete(usuario),
      );
    ...
    (error) => {
      ...
      // Reintentar la suscripción después de un error
      setTimeout(
        () => suscribirseAUsuariosPendientes(callback, opciones),
        5000,
      );
    },
  );
 ...
 * @returns Función para cancelar la suscripción
 */
export const suscribirseAResidenciales = (
  callback: (residenciales: Residencial[]) => void,
) => {
  const q = query(
    collection(db, "residenciales"),
    orderBy("nombre", "asc"),
 ...
 */
export const esAdministradorDeResidencial = async (
  residencialId: string,
): Promise<boolean> => {
  try {
    const auth = await getAuthSafe();
    ...
    const adminRef = doc(
      db,
      `residenciales/${residencialId}/admins/${auth.currentUser.uid}`,
    );
    ...
    if (residencialSnap.exists()) {
      ...
      if (
        residencialData.administradores &&
    ...
    const userResidencialRef = doc(
      db,
      `usuarios/${auth.currentUser.uid}/residenciales/${residencialId}`,
    );
...
export const getNotificaciones = async (residencialID: string) => {
  try {
    console.log(
      `🔍 getNotificaciones: Buscando notificaciones para residencial ID: ${residencialID}`,
    );

    const notificacionesRef = collection(
      db,
      `residenciales/${residencialID}/notificaciones`,
    );
    const snapshot = await getDocs(notificacionesRef);
    console.log(
      `📊 Subcolección 'notificaciones': ${snapshot.docs.length} documentos`,
    );
...
export const suscribirseANotificaciones = (
  residencialID: string,
  callback: (notificaciones: AlertaPanico[]) => void,
) => {
  try {
    console.log(
      `🔔 suscribirseANotificaciones: Iniciando suscripción para residencial ID: ${residencialID}`,
    );
    ...
    return onSnapshot(
      ...
      (snapshot) => {
        console.log(
          `📊 Suscripción a notificaciones: ${snapshot.docs.length} documentos`,
        );
        ...
        if (snapshot.docs.length > 0) {
          ...
          console.log(
            `✅ Suscripción: Enviando ${notificaciones.length} notificaciones al componente`,
          );
 ...
 */
export const getNotificacionesProgramadas = async (
  residencialID: string,
): Promise<NotificacionProgramada[]> => {
  try {
    const ref = collection(
      db,
      `residenciales/${residencialID}/notificacionesProgramadas`,
    );
    const q = query(ref, orderBy("scheduledAt", "asc"));
    const snapshot = await getDocs(q);
    return snapshot.docs.map(
      (d) => ({ id: d.id, ...d.data() }) as NotificacionProgramada,
    );
  } catch (error) {
...
export const suscribirseANotificacionesProgramadas = (
  ...
  try {
    ...
    return onSnapshot(
      ...
      (error) => {
        console.error(
          "Error en suscripción a notificacionesProgramadas:",
          error,
        );
      },
    );
...
export const getPagos = async (residencialID: string): Promise<Pago[]> => {
  try {
    console.log(
      `🔍 getPagos: Buscando pagos para residencial ID: ${residencialID}`,
    );
    ...
    for (const nombreColeccion of coleccionesPosibles) {
      ...
      if (snapshot.docs.length > 0) {
        ...
        console.log(
          `✅ Se encontraron ${snapshot.docs.length} pagos en la subcolección '${nombreColeccion}'`,
        );
    ...
    console.log(
      `⚠️ No se encontraron pagos en ninguna subcolección para el residencial: ${residencialID}`,
    );
    return [];
  } catch (error) {
 ...
 */
export const suscribirseAPagos = (
  residencialID: string,
  callback: (pagos: Pago[]) => void,
): (() => void) => {
  const pagosRef = collection(
    db,
    `residenciales/${residencialID}/paymentIntents`,
  );
  ...
  const unsubscribe = onSnapshot(
    ...
    (error) => {
      console.error(
        `Error al suscribirse a pagos para el residencial ${residencialID}:`,
        error,
      );
      toast.error(
        `Error al obtener pagos en tiempo real para ${residencialID}`,
      );
    },
  );
 ...
 * @returns Función para cancelar la suscripción.
 */
export const suscribirseAIngresos = (
  residencialDocId: string,
  callback: (ingresos: Ingreso[]) => void,
): (() => void) => {
  if (!residencialDocId) {
    console.error(
      "Error: residencialDocId es indefinido en suscribirseAIngresos",
    );
    return () => {};
  }
  try {
    ...
    const unsubscribe = onSnapshot(
      ...
      (error) => {
        ...
        toast.error(
          `Error al obtener ingresos en tiempo real para ${residencialDocId}.`,
        );
        callback([]);
      },
    );
...
export const getVehicleHistory = async (
  ...
  if (!residencialDocId || !placa) {
    console.error(
      "Error: residencialDocId o placa es indefinido en getVehicleHistory",
    );
    return null;
  }
  ...
  try {
    console.log(
      `🔍 getVehicleHistory: Buscando historial para placa ${placa} en residencial ${residencialDocId}`,
    );

    const docRef = doc(
      db,
      `residenciales/${residencialDocId}/vehicle_history/${placa}`,
    );
    ...
    if (docSnap.exists()) {
      ...
      const totalIngresos = personas.reduce(
        (sum: number, persona: any) => sum + (persona.totalEntries || 0),
        0,
      );
      ...
      if (personas.length > 0) {
        const allEntries = personas
          .flatMap((persona: any) => [persona.firstEntry, persona.lastEntry])
          .filter(Boolean)
      ...
      const vehicleHistory: VehicleHistory = {
        id: placa,
        placa: placa,
        vehicleInfo: firstEntry.vehicleInfo || {
          placa,
          color: "N/A",
          marca: "N/A",
          modelo: "N/A",
      ...
      console.log(
        `✅ Historial construido en vivo para placa ${placa} con ${ingresos.length} ingresos.`,
      );
      return vehicleHistory;
    }
  } catch (error: any) {
    console.error(
      `❌ Error al obtener historial del vehículo ${placa}:`,
      error,
    );
    ...
    if (
      ...
      console.error(
        "ÍNDICE FALTANTE: Por favor, crea el siguiente índice compuesto en Firestore:",
      );
      console.error(url);
      toast.error(
        "Se requiere una configuración en la base de datos. Contacta a soporte.",
      );
 ...
 */
export const getAllVehicleHistories = async (
  residencialDocId: string,
): Promise<VehicleHistory[]> => {
  if (!residencialDocId) {
    console.error(
      "Error: residencialDocId es indefinido en getAllVehicleHistories",
    );
    return [];
  }
  ...
  try {
    console.log(
      `🔍 getAllVehicleHistories: Buscando todos los historiales para residencial ${residencialDocId}`,
    );

    const vehicleHistoriesRef = collection(
      db,
      `residenciales/${residencialDocId}/vehicle_history`,
    );
    const q = query(vehicleHistoriesRef, orderBy("lastUpdated", "desc"));
    const snapshot = await getDocs(q);

    console.log(
      `📊 Se encontraron ${snapshot.docs.length} historiales de vehículos para residencial ${residencialDocId}`,
    );
    ...
    console.error(
      `❌ Error al obtener historiales de vehículos para residencial ${residencialDocId}:`,
      error,
    );
...
export const registrarPagoEfectivo = async (
  data: EfectivoPaymentData,
): Promise<void> => {
  const {
    residencialId,
    userId,
  ...
  if (!residencialId) {
    throw new Error(
      "El ID del residencial es requerido para registrar un pago.",
    );
  }

  try {
    console.log(
      "🚀 [V2] Registrando pago en efectivo a través de Cloud Function (Atómico)...",
    );
    const registerFn = httpsCallable<
      { residencialId: string; data: any },
      { success: boolean; paymentId: string }
    >(functions, "apiRegisterDirectCashPayment");
    ...
    const result = await registerFn({
      ...
      data: {
        houseId:
          calle && houseNumber
            ? `${calle}-${houseNumber}`.toLowerCase().replace(/\s+/g, "_")
            : data.userId,
        amount: amount,
    ...
    console.log(
      `✅ [V2] Pago registrado exitosamente con ID: ${result.data.paymentId}`,
    );
    return;
  } catch (error: any) {
...
export const getAllUsuarios = async (opciones?: {
  ...
  try {
    ...
    if (debug)
      console.log(
        "🔍 Obteniendo TODOS los usuarios con paginación automática...",
      );
    ...
    try {
      if (debug)
        console.log("🔄 Intentando obtener todos los usuarios de una vez...");
      const snapshot = await getDocs(usuariosRef);
      totalUsuarios = snapshot.size;

      if (debug)
        console.log(`✅ Obtenidos ${totalUsuarios} usuarios de una vez`);
      ...
      if (debug)
        console.log(
          "⚠️ No se pudieron obtener todos los usuarios de una vez, usando paginación...",
        );
      ...
      while (true) {
        ...
        if (debug)
          console.log(
            `📦 Lote ${totalBatches} obtenido: ${batchUsuarios.length} usuarios`,
          );
      ...
      if (debug)
        console.log(
          `✅ Total de usuarios obtenidos: ${allUsuarios.length} en ${totalBatches} lotes`,
        );
      return allUsuarios;
    }
  } catch (error) {
