...
export async function updateTagStatusDirect(
  ...
  try {
    ...
    await addDoc(
      collection(db, "residenciales", residencialDocId, "auditLogs"),
      {
        type: "TAG_STATUS_CHANGE",
        tagId: tagId,
...
export async function getTagsSync(residencialDocId: string): Promise<Tag[]> {
  try {
    console.log(
      `🏷️ [getTagsSync] Obteniendo tags del residencialDocId: ${residencialDocId}`,
    );
    ...
    try {
      ...
      console.log(
        `🏷️ [getTagsSync] Consulta con orderBy por cardNumberDec exitosa`,
      );
    } catch (orderByError) {
      console.log(
        `🏷️ [getTagsSync] Error con orderBy, usando consulta simple:`,
        orderByError,
      );
    ...
    console.log(
      `🏷️ [getTagsSync] Documentos encontrados: ${snapshot.docs.length}`,
    );
    ...
    console.error(
      `🏷️ [getTagsSync] Error obteniendo tags del residencialDocId ${residencialDocId}:`,
      error,
    );
    throw error;
  }
