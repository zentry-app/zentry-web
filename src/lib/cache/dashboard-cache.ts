 ...
 */
export const getCachedResidencialUsers = async (
...
export const getCachedIngresos = async (
  ...
  const cacheKey = `accessEvents-${residencialDocId}-${startDate.getTime()}`;
  ...
  return getCachedQuery(
    ...
    async () => {
      ...
      const constraints: any[] = [
        where("entryAt", ">=", startDate),
        orderBy("entryAt", "desc"),
        fbLimit(1000),
      ];

      if (endDate) {
        constraints.splice(1, 0, where("entryAt", "<=", endDate));
      }
 ...
 */
export const getCachedPendingReservations = async (
  residencialDocId: string,
) => {
  return getCachedQuery(
