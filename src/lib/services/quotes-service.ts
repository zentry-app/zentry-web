...
export class QuotesService {
  ...
  static async getAllClients(): Promise<QuoteClient[]> {
    const q = query(collection(db, CLIENTS_COLLECTION), orderBy("empresa"));
    const snapshot = await getDocs(q);
    return snapshot.docs.map((doc) => ({
      id: doc.id,
      ...doc.data(),
  ...
  static async createClient(
    data: Omit<QuoteClient, "id" | "createdAt" | "updatedAt">,
  ): Promise<string> {
  ...
  static async createConcept(
    data: Omit<QuoteConcept, "id" | "createdAt" | "updatedAt">,
  ): Promise<string> {
  ...
  static async getAllTerms(): Promise<QuoteTerm[]> {
    const q = query(collection(db, TERMS_COLLECTION), orderBy("orden"));
    const snapshot = await getDocs(q);
    return snapshot.docs.map((doc) => ({
      id: doc.id,
      ...doc.data(),
  ...
  static async createTerm(
    data: Omit<QuoteTerm, "id" | "createdAt" | "updatedAt">,
  ): Promise<string> {
  ...
  static async seedDefaults(
    ...
    const conceptsCount = await getCountFromServer(
      collection(db, CONCEPTS_COLLECTION),
    );
    if (conceptsCount.data().count === 0) {
      for (const concept of defaultConcepts) {
        await this.createConcept(concept as any);
      }
    }

    const termsCount = await getCountFromServer(
      collection(db, TERMS_COLLECTION),
    );
    if (termsCount.data().count === 0) {
