...
export const SupportService = {
  ...
  getTickets: async (
    ...
    try {
      ...
      if (residencialId) {
        ...
        const supportTicketsRef = collection(
          db,
          "residenciales",
          residencialDoc.id,
          "supportTickets",
        ...
        if (filters?.categoria && filters.categoria.length > 0) {
          ticketsQuery = query(
            ticketsQuery,
            where("categoria", "in", filters.categoria),
          );
        }
        if (filters?.prioridad && filters.prioridad.length > 0) {
          ticketsQuery = query(
            ticketsQuery,
            where("prioridad", "in", filters.prioridad),
          );
        }
        ...
        for (const residencialDoc of residencialesDocs) {
          ...
          const residencialData = residencialDoc.data() as
            | { residencialID?: string }
            | undefined;
          const residencialCode =
            residencialData?.residencialID ?? residencialDoc.id;
          const ticketsSnapshot = await getDocs(ticketsQuery);
          ticketsSnapshot.forEach((doc) => {
            const data = doc.data();
            tickets.push({
      ...
      if (filters?.fechaDesde || filters?.fechaHasta) {
        filteredTickets = filteredTickets.filter((ticket) => {
          const fechaCreacion =
            ticket.fechaCreacion instanceof Timestamp
      ...
      if (filters?.soloVencidos) {
        ...
        filteredTickets = filteredTickets.filter((ticket) => {
          ...
          if (ticket.estado === "resuelto" || ticket.estado === "cerrado")
            return false;
          const fecha =
            ticket.fechaResolucionPrevista instanceof Timestamp
  ...
  getTicketRespuestas: async (
    ...
    try {
      ...
      return snapshot.docs.map(
        (d) => ({ respuestaId: d.id, ...d.data() }) as TicketRespuesta,
      );
    } catch (error) {
  ...
  addTicketRespuesta: async (
    ...
    try {
      ...
      if (estado === "resuelto" || estado === "cerrado") {
        throw new Error(
          "Este ticket está cerrado. El usuario debe crear un nuevo ticket.",
        );
      }
    ...
    try {
      ...
      const stats: TicketStats = {
        ...
        vencidos: tickets.filter((t) => {
          ...
          const fecha =
            t.fechaResolucionPrevista instanceof Timestamp
      ...
      if (ticketsResueltos.length > 0) {
        const tiemposResolucion = ticketsResueltos.map((ticket) => {
          ...
          const fechaResolucion =
            ticket.fechaResolucion instanceof Timestamp
        ...
        const tiemposPrimeraResp = ticketsConPrimeraResp.map((ticket) => {
          const fechaCreacion =
            ticket.fechaCreacion instanceof Timestamp
              ? ticket.fechaCreacion.toDate()
              : new Date(ticket.fechaCreacion as string);
          const fechaPrimeraResp =
            ticket.fechaPrimeraRespuesta instanceof Timestamp
              ? ticket.fechaPrimeraRespuesta.toDate()
              : new Date(ticket.fechaPrimeraRespuesta as string);
          return (
            (fechaPrimeraResp.getTime() - fechaCreacion.getTime()) /
            (1000 * 60 * 60)
          );
        });
        stats.tiempoPromedioPrimeraRespuesta =
          tiemposPrimeraResp.reduce((a, b) => a + b, 0) /
          tiemposPrimeraResp.length;
      }
  ...
  getKnowledgeBase: async (): Promise<KnowledgeBaseItem[]> => {
    try {
      const snapshot = await getDocs(
        query(
          collection(db, "supportKnowledgeBase"),
          orderBy("fechaCreacion", "desc"),
        ),
      );
    ...
    try {
      ...
      if (item.pregunta && item.respuesta) {
        try {
          ...
          console.log(
            `Embedding generado para pregunta frecuente: ${finalItemId}`,
          );
        } catch (embeddingError) {
          // No fallar si el embedding falla, solo loguear
          console.warn(
            "Error generando embedding (no crítico):",
            embeddingError,
          );
  ...
  getKnowledgeBaseStats: async (): Promise<KnowledgeBaseStats> => {
    try {
      ...
      console.error(
        "Error obteniendo estadísticas de base de conocimiento:",
        error,
      );
      throw error;
    }
  ...
  regenerateEmbedding: async (
    ...
    try {
      if (!item.pregunta || !item.respuesta) {
        throw new Error(
          "Se requiere pregunta y respuesta para generar embedding",
        );
      }
