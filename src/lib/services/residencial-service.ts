...
export const ResidencialService = {
   ...
   */
  createResidencial: async (
    residencialData: Omit<Residencial, "id" | "createdAt" | "updatedAt">,
  ): Promise<string> => {
    try {
      const residencialesRef = collection(db, "residenciales");
   ...
   */
  updateResidencial: async (
    id: string,
    residencialData: Partial<Residencial>,
  ): Promise<void> => {
    try {
      const residencialDocRef = doc(db, "residenciales", id);
  ...
  getResidencialesByAdmin: async (adminId: string): Promise<Residencial[]> => {
    try {
      ...
      console.error(
        "Error al obtener residenciales por administrador:",
        error.message,
      );
      throw error;
    }
  ...
  /**
   * @deprecated No callers found — use getCachedResidenciales() or a one-time getDocs instead. Opens onSnapshot on entire collection.
   */
  subscribeToResidenciales: (
   ...
   */
  subscribeToResidencialesByAdmin: (
    adminId: string,
    callback: (residenciales: Residencial[]) => void,
  ) => {
    const residencialesRef = collection(db, "residenciales");
