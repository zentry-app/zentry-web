...
export class CashPaymentService {
  ...
  static async registerCashPayment(
    residencialId: string,
    paymentData: Omit<CashPayment, "id" | "registradoPor"> & {
      // fechaPago es opcional aquí (en el modelo es requerido pero se llena
      // server-side cuando no viene). Cuando el caller lo provee, se respeta
      // como la fecha real del cobro físico → llega al backend como dateStr.
      fechaPago?: Timestamp | Date;
    },
  ): Promise<string> {
    try {
      console.log(
        `💰 Registrando pago en efectivo para ${paymentData.userName}`,
      );

      console.log(
        "🚀 Registrando pago en efectivo a través de Cloud Function (Atómico)...",
      );
      ...
      console.log(
        `✅ Pago registrado exitosamente con ID: ${result.data.paymentId}`,
      );
  ...
  static async getCashPayments(
    ...
    try {
      console.log(
        `🔍 Obteniendo pagos en efectivo para residencial: ${residencialId} — [paymentIntents SoT]`,
      );
      ...
      console.log(
        `🔍 [CASH] Query ejecutado. Documentos encontrados: ${querySnapshot.docs.length}`,
      );
  ...
  static async getCashPaymentsByHouse(
    ...
    try {
      console.log(
        `🏠 Obteniendo pagos en efectivo para casa: ${calle} #${houseNumber} — [paymentIntents SoT]`,
      );
      ...
      console.log(
        `✅ ${pagos.length} pagos en efectivo obtenidos para la casa`,
      );
      return pagos;
    } catch (error) {
   ...
   */
  static async getCashPaymentStats(
    residencialId: string,
  ): Promise<CashPaymentStats> {
    try {
      ...
      const pagosEsteMes = pagos.filter((pago) => {
        const fechaPago =
          pago.fechaPago instanceof Timestamp
      ...
      console.error(
        "❌ Error al obtener estadísticas de pagos en efectivo:",
        error,
      );
      throw error;
    }
  ...
  > {
    try {
      ...
      // Crear resumen
      const resumen = Array.from(pagosPorCasa.entries()).map(
        ([casa, pagosCasa]) => {
          const totalRecaudado = pagosCasa.reduce(
            (sum, p) => sum + p.amount,
            0,
          );
          const ultimoPago = pagosCasa
            .map((p) =>
              p.fechaPago instanceof Timestamp
                ? p.fechaPago.toDate()
          ...
          const estado = (
            ultimoPago && ultimoPago > haceUnMes ? "al_dia" : "pendiente"
          ) as "al_dia" | "pendiente" | "moroso";
  ...
  static watchCashPayments(
    ...
    console.log(
      `🔔 Suscribiéndose a intents de pago en efectivo para residencial: ${residencialId} — [paymentIntents SoT]`,
    );
    ...
    return onSnapshot(q, (querySnapshot) => {
      ...
      console.log(
        `📣 Actualización en tiempo real — [paymentIntents:cash]: ${pagos.length} pagos`,
      );
      callback(pagos);
    });
  }
   ...
   */
  static async deleteCashPayment(
    _residencialId: string,
    _paymentId: string,
  ): Promise<void> {
    throw new Error(
      "El borrado directo de paymentIntents está deshabilitado. Use el flujo oficial v2 en backend.",
    );
  }
}
