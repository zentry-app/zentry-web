...
const getAlertasPanicoActivas = async (): Promise<PanicAlert[]> => {
  try {
    console.log(
      "[DashboardService] Buscando alertas de pánico activas usando lógica de la página de alertas",
    );
    ...
    console.log(
      `[DashboardService] Encontrados ${residencialesDocs.length} residenciales`,
    );
    ...
    for (const residencialDoc of residencialesDocs) {
      ...
      try {
        ...
        const alertasRef = collection(
          db,
          `residenciales/${residencialDoc.id}/alertas`,
        );
        ...
        console.log(
          `[DashboardService] Residencial ${residencialNombre}: ${alertasSnapshot.docs.length} alertas activas encontradas`,
        );
        ...
        console.error(
          `[DashboardService] Error buscando alertas en residencial ${residencialNombre}:`,
          error,
        );
        continue;
      }
    ...
    if (alertasEncontradas.length === 0) {
      console.log(
        "[DashboardService] No se encontraron alertas en subcolecciones, buscando en colecciones raíz...",
      );
      ...
      for (const nombreColeccion of coleccionesPosibles) {
        try {
          ...
          console.log(
            `[DashboardService] Colección raíz '${nombreColeccion}': ${snapshot.docs.length} alertas encontradas`,
          );
          ...
          console.error(
            `[DashboardService] Error buscando en colección '${nombreColeccion}':`,
            error,
          );
          continue;
        }
    ...
    alertasEncontradas.sort(
      (a, b) => b.timestamp.getTime() - a.timestamp.getTime(),
    );

    console.log(
      `[DashboardService] Total de alertas de pánico activas encontradas: ${alertasEncontradas.length}`,
    );
    return alertasEncontradas.slice(0, 10); // Limitar a 10 para el dashboard
  } catch (error: any) {
    console.error(
      "[DashboardService] Error al obtener alertas de pánico:",
      error.message,
    );
    return [];
  }
...
const DashboardService = {
  ...
  getRealTimeStats: async (): Promise<DashboardRealTimeStats> => {
    try {
      console.log(
        "[DashboardService] Obteniendo estadísticas en tiempo real...",
      );
      ...
      const [ingresosActivosSnap, eventosHoySnap, ingresosRecientesSnap] =
        await Promise.all([
      ...
      console.error(
        "[DashboardService] Error al obtener estadísticas:",
        error.message,
      );
      return {
        alertasPanico: 0,
  ...
  getPendingPanicAlerts: async (): Promise<PanicAlert[]> => {
    console.log(
      "[DashboardService] Obteniendo alertas de pánico pendientes...",
    );
      ...
      const visitors: ActiveVisitor[] = querySnapshot.docs.map((doc) => {
        ...
        return {
          ...
          residentName:
            ...
            "Residente",
          entryTime:
            data.timestamp?.toDate() || data.entryTime?.toDate() || new Date(),
          expectedDuration:
            data.expectedDuration || data.duracionEsperada || "2 horas",
          status: data.status || "active",
        };
      });

      console.log(
        `[DashboardService] ${visitors.length} visitantes activos obtenidos`,
      );
      return visitors;
    } catch (error: any) {
      console.error(
        "[DashboardService] Error al obtener visitantes activos:",
        error.message,
      );
      return [];
    }
  ...
  getActiveRounds: async (): Promise<ActiveRound[]> => {
    try {
      console.log(
        "[DashboardService] Rondas de seguridad no disponibles en este momento",
      );
      return [];
    } catch (error: any) {
      console.error(
        "[DashboardService] Error al obtener rondas activas:",
        error.message,
      );
      return [];
    }
  ...
  getRecentActivity: async (): Promise<RecentActivity[]> => {
    try {
      ...
      console.log(
        `[DashboardService] ${recentActivities.length} actividades recientes obtenidas`,
      );
      return recentActivities;
    } catch (error: any) {
      console.error(
        "[DashboardService] Error al obtener actividad reciente:",
        error.message,
      );
      return [];
    }
  ...
  getSystemHealth: async (): Promise<SystemHealth> => {
    try {
      ...
      console.error(
        "[DashboardService] Error al obtener estado del sistema:",
        error.message,
      );
      return {
        uptime: "0h",
  ...
  subscribeToRealTimeStats: (
    onUpdate: (stats: DashboardRealTimeStats) => void,
  ) => {
    console.log(
      "[DashboardService] Configurando suscripciones en tiempo real...",
    );
    ...
    const unsubIngresosActivos = onSnapshot(
      ingresosActivosQuery,
      (snapshot) => {
    ...
    const unsubIngresosRecientes = onSnapshot(
      ingresosRecientesQuery,
      (snapshot) => {
